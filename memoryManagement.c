...
frameList_t emptyFrameListTail = NULL;	//	Ende der LEEREN Framelist
...
/*	Allokiert dem Prozess eine (proportionale) Anzahl von Rahmen.
	Gibt es nicht genug Rahmen kann der Prozess nicht starten.	*/
{
	...
	if (ttlFrame > emptyFrameCounter) {
		...
		free(processTable[pid].pageTable);		// befreit den ProzessTabelle, die voreher erstellt war
		return FALSE;
	}
	if (processTable[pid].size < initFrames) {	// falls der Prozess kleiner als der mindestanzahl an Rahmen ist
		ttlFrame = processTable[pid].size;
		for (int i = 0; i < ttlFrame; i++) {	// wird ihm genau so viel Rahmen wie ihrem Größe in Seiten zugeordnet
	...
	Interval referenziert wurden.*/
{
	...
	while (iterator != NULL) {		
		if ((iterator->residentPage->agingVal & point)) { // z.b (01010001b) & (10000000b) = 0100000b = 64 => TRUE
	...
	if (point >> 1) {   // wenn point noch nicht 00000001b ist, rufe die Funktion auf die Sublisten ab, übergebe point mit eine bitstelle verschoben
		frameList_t lhs = sortUsedFrameList(point >> 1, zeroes); // Rekursiver Aufruf der Funktion, wobei die Nächste Bitstelle überprüft wird
		frameList_t rhs = sortUsedFrameList(point >> 1, ones);
		if (lhs != NULL) {		// für die 'linke' zurückgegebene Liste
			iterator = lhs;		
			if (rhs == NULL) {	// wenn die 'rechte' liste leer ist
				return lhs;		// einfach die 'linke' liste zurückgeben
			}
			while (iterator->next != NULL) { 
				iterator = iterator->next; // iterieren wir durch
			}
			iterator->next = rhs; //und füge die 'rechte' zurückgegebene Liste am Ende
			return lhs;	// liefern wir den zurück
		}
		else { // falls die 'linke' liste leer ist, einfach dem 'rechten' zurückgeben
			return rhs;
		}
	}
	if (zeroes != NULL) { 
		zeroesLast->next = ones; // hänge die ones teilliste am Ende der zeroes teilliste
		return zeroes; // liefert die zeroes teilliste zurück
	}
	else { // falls die zeroes teilliste leer ist
		return ones;	// die ones teilliste zurückgeben
	}
}
Boolean movePageIn(unsigned pid, unsigned page, unsigned frame)
/* liefert TRUE wenn Erfolgreich, andernfalls FALSE 							*/
{
	processTable[pid].pageTable[page].frame = frame;		
	processTable[pid].pageTable[page].present = TRUE;
	// Seite eingelagert, R- und M-Bits zurücksetzen
